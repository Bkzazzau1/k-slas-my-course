...
class _ExamSetupViewState extends State<ExamSetupView> {
  ...
  Widget build(BuildContext context) {
    ...
    final drawingPolicy = DrawingRequirementService.policyForCourse(courseCode ?? 'CSC 305');
    final whiteboardEnabled = isGraded && drawingPolicy.whiteboardEnabledForGraded;
    final totalQuestions = (objective ? objectiveCount : 0) + (fillBlank ? fillCount : 0) + (theory ? theoryCount : 0);
          ...
          children: [
            ...
            _GlassCard(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _InfoBar(
                  icon: isGraded ? Icons.security_rounded : Icons.school_outlined,
                  text: isGraded
                      ? hasTemplate
            ...
            if (drawingPolicy.whiteboardEnabledForGraded) ...[
              _GlassCard(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  if (drawingPolicy.prompt != null && drawingPolicy.prompt!.trim().isNotEmpty) ...[
                    const SizedBox(height: 8),
                    Text(drawingPolicy.prompt!.trim(), style: _mutedStyle(context)),
                  ],
                ]),
              ),
              const SizedBox(height: 12),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Wrap(spacing: 8, runSpacing: 8, children: [
                  _ChoicePill(
                    ...
                    selected: deliveryMode == ExamDeliveryMode.remoteProctored,
                    onTap: settingsLocked ? () {} : () => setState(() => deliveryMode = ExamDeliveryMode.remoteProctored),
                  ),
                ]),
                const SizedBox(height: 10),
                Text(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Wrap(spacing: 8, runSpacing: 8, children: [
                  _ChoicePill(
                    ...
                    onTap: settingsLocked ? () {} : () => setState(() => mode = ExamMode.simulation),
                  ),
                  _ChoicePill(
                    label: 'Practice',
                    selected: mode == ExamMode.practice,
                    onTap: settingsLocked ? () {} : () => setState(() => mode = ExamMode.practice),
                  ),
                ]),
                const SizedBox(height: 10),
                Text(
                  settingsLocked ? 'Mode is locked by lecturer backend settings.' : 'Simulation behaves like a real exam; practice gives more learning flexibility.',
                  style: _mutedStyle(context),
                ),
              ]),
            ),
            ...
            _GlassCard(
              ...
              child: _PremiumDropdown<String>(
                value: topic,
                items: ['Mixed', 'WeakOnly', ...topics.where((item) => item != 'Mixed')],
                labelBuilder: (value) => value == 'WeakOnly' ? 'Weak topics only' : value,
                onChanged: (value) {
            ...
            if (settingsLocked) ...[
              const SizedBox(height: 8),
              Text('Topic, question counts, and timer are locked by lecturer backend settings.', style: _mutedStyle(context)),
            ],
            const SizedBox(height: 12),
            _GlassCard(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                if (settingsLocked) ...[
                  if (objective) _LockedCountRow(label: 'Objective questions', value: objectiveCount),
                  if (fillBlank) _LockedCountRow(label: 'Fill-blank questions', value: fillCount),
                  if (theory) _LockedCountRow(label: 'Theory questions', value: theoryCount),
                ] else ...[
                  if (objective) _SliderBlock(label: 'Objective questions', value: objectiveCount, min: 5, max: 60, onChanged: (value) => setState(() => objectiveCount = value)),
                  if (fillBlank) _SliderBlock(label: 'Fill-blank questions', value: fillCount, min: 1, max: 40, onChanged: (value) => setState(() => fillCount = value)),
                  if (theory) _SliderBlock(label: 'Theory questions', value: theoryCount, min: 1, max: 10, onChanged: (value) => setState(() => theoryCount = value)),
                ],
              ]),
            ),
            const SizedBox(height: 12),
            ...
            _PrimaryCta(
              label: isGraded ? 'Start Graded Examination' : 'Start Ungraded Simulation',
  ...
  Future<void> _startExam() async {
    final gradedTemplate = isGraded
        ? (_gradedTemplate ?? GradedSessionTemplateService.templateFor(courseCode: courseCode!, sessionType: SessionType.examination))
        : null;

    if (isGraded && gradedTemplate == null) {
      Get.snackbar('Graded template missing', 'Lecturer has not published graded examination settings yet.', snackPosition: SnackPosition.BOTTOM);
      return;
    }
    ...
    final effectiveObjectiveCount = gradedTemplate?.objectiveQuestions ?? (effectiveObjective ? objectiveCount : 0);
    final effectiveFillCount = gradedTemplate?.fillBlankQuestions ?? (effectiveFillBlank ? fillCount : 0);
    final effectiveTheoryCount = gradedTemplate?.theoryQuestions ?? (effectiveTheory ? theoryCount : 0);
    ...
    if (!effectiveObjective && !effectiveFillBlank && !effectiveTheory) {
      Get.snackbar('Select a type', 'Choose at least one question type.', snackPosition: SnackPosition.BOTTOM);
      return;
    }
    ...
    final drawingPolicy = DrawingRequirementService.policyForCourse(courseCode!);
    final whiteboardEnabled = isGraded && drawingPolicy.whiteboardEnabledForGraded;
    ...
    final cfg = ExamConfig(
      ...
      questionSource: isGraded ? QuestionSourceType.lecturerAdmin : QuestionSourceType.studentLocal,
      whiteboardEnabled: whiteboardEnabled,
    ...
    await proctoring.startExamSequence(examId, onVerified: () => Get.toNamed(Routes.examRun, arguments: cfg));
  }
}
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Text('$courseCode Examination', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 22))),
          const Icon(Icons.verified_user_rounded, color: Colors.white),
        ]),
        const SizedBox(height: 12),
        Text(
          isGraded ? 'High-stakes exam gateway with verification, readiness checks, and proctoring rules.' : 'Ungraded exam simulation for student practice only.',
  ...
  Widget build(BuildContext context) {
    return _GlassCard(
      ...
      child: Column(children: [
        ...
        _CheckRow(label: 'Camera/audio/environment scan required', done: proctoringActive && deliveryMode == ExamDeliveryMode.remoteProctored),
        _CheckRow(label: 'Whiteboard enabled when required', done: whiteboardEnabled || !proctoringActive),
        const SizedBox(height: 8),
        _InfoBar(
          icon: readyToStart ? Icons.verified_rounded : Icons.warning_amber_rounded,
          text: readyToStart ? 'Ready to continue. Review the rules before starting.' : 'Not ready yet. Wait for the lecturer/exam officer to publish exam settings.',
          danger: !readyToStart,
        ),
      ]),
    );
...
class _VerificationSteps extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _GlassCard(
      ...
      child: Column(children: [
        _StepRow(number: 1, title: 'Identity and candidate session', subtitle: 'Confirm course, exam mode, and student record.'),
        _StepRow(number: 2, title: 'Device and network check', subtitle: 'Prepare a stable device and avoid switching apps.'),
        _StepRow(number: 3, title: isGraded ? 'Camera/audio/environment scan' : 'Practice readiness', subtitle: isGraded ? 'Room scan and integrity checks run before entry.' : 'No proctoring is requested in simulation.'),
        _StepRow(number: 4, title: 'Start exam', subtitle: 'Timer starts only after the final gateway is completed.'),
      ]),
    );
  }
}
...
class _ExamRulesCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _GlassCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _RuleBullet(text: 'Keep this app open until submission is completed.'),
        _RuleBullet(text: 'Submit before the $minutes-minute timer expires.'),
        _RuleBullet(text: 'Saved answers are tracked locally for recovery during poor network.'),
        if (isGraded) ...[
          _RuleBullet(text: 'Do not leave the camera/audio verification environment.'),
          _RuleBullet(text: 'Multiple faces, phone visibility, or app switching may affect integrity score.'),
        ],
      ]),
    );
  }
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(blurRadius: 18, offset: const Offset(0, 10), color: cs.shadow.withValues(alpha: 0.04))],
          ),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
...
class _CheckRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(children: [
        Icon(done ? Icons.check_circle_rounded : Icons.pending_rounded, color: color, size: 20),
        const SizedBox(width: 10),
        Expanded(child: Text(label, style: TextStyle(color: cs.onSurface, fontWeight: FontWeight.w800))),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        CircleAvatar(radius: 15, backgroundColor: cs.primary.withValues(alpha: 0.12), child: Text('$number', style: TextStyle(color: cs.primary, fontWeight: FontWeight.w900))),
        const SizedBox(width: 10),
...
class _RuleBullet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: const EdgeInsets.only(bottom: 9),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(padding: const EdgeInsets.only(top: 7), child: Container(width: 6, height: 6, decoration: BoxDecoration(color: cs.primary, shape: BoxShape.circle))),
        const SizedBox(width: 10),
        Expanded(child: Text(text, style: TextStyle(color: cs.onSurface.withValues(alpha: 0.72), fontWeight: FontWeight.w700, height: 1.32))),
      ]),
    );
  }
}
...
class _InfoBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: color.withValues(alpha: 0.09), borderRadius: BorderRadius.circular(16), border: Border.all(color: color.withValues(alpha: 0.16))),
      child: Row(children: [
...
class _ChoicePill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ChoiceChip(
      ...
      labelStyle: TextStyle(color: selected ? cs.primary : cs.onSurface, fontWeight: FontWeight.w900),
    );
  }
}

class _SwitchRow extends StatelessWidget {
  const _SwitchRow({required this.title, required this.subtitle, required this.value, required this.enabled, required this.onChanged});
  final String title;
  final String subtitle;
  final bool value;
  final bool enabled;
...
class _PremiumDropdown<T> extends StatelessWidget {
  const _PremiumDropdown({required this.value, required this.items, required this.labelBuilder, required this.onChanged});
  final T value;
  final List<T> items;
...
class _SliderBlock extends StatelessWidget {
  const _SliderBlock({required this.label, required this.value, required this.min, required this.max, required this.onChanged});
  final String label;
  final int value;
  final int min;
  final int max;
...
class _PrimaryCta extends StatelessWidget {
  const _PrimaryCta({required this.label, required this.onTap, required this.subText, required this.enabled});
  final String label;
  final VoidCallback onTap;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      const SizedBox(height: 8),
      Text(subText, textAlign: TextAlign.center, style: TextStyle(color: cs.onSurface.withValues(alpha: 0.70), fontWeight: FontWeight.w600)),
    ]);
  }
}
