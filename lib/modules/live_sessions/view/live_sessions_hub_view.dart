...
class _LiveSessionsHubViewState extends State<LiveSessionsHubView> {
  ...
  Widget build(BuildContext context) {
    ...
    final registeredCourses =
        profile?.selectedCourses.map((e) => e.trim().toUpperCase()).toSet() ??
        const <String>{};
                    ...
                    controller.sessions
                      ...
                      ..sort(
                        (a, b) => _sessionWeight(
                          a,
                          now,
                        ).compareTo(_sessionWeight(b, now)),
                      );
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      _HistoryShortcutCard(
                        onTap: () => Get.toNamed(Routes.liveClassHistory),
                      ),
                      const SizedBox(height: 12),
  ...
  List<LiveSessionModel> _filterSessions(
  ...
  void _joinSession(LiveSessionModel session, dynamic profile) {
    final now = DateTime.now();
    final route = session.isCompletedAt(now)
        ? Routes.liveSessionReplay
    ...
    Get.toNamed(
      ...
      arguments: {
        ...
        'registrationNumber':
            profile?.matricNo ?? profile?.email ?? 'student-demo',
      },
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Join lectures, prepare before class, review materials, and keep attendance up to date.',
            style: TextStyle(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'View past classes, attendance, notes saved, and replay availability.',
                    style: TextStyle(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          const _CheckLine(
            text: 'Use a stable network before joining live class.',
          ),
          const _CheckLine(
            text: 'Keep microphone muted until the lecturer allows questions.',
          ),
          const _CheckLine(
            text: 'Join with your registered student profile for attendance.',
          ),
        ],
      ),
    );
...
class _LiveClassCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tone = session.isLiveAt(now)
        ? Colors.green.shade700
        : session.isUpcomingAt(now)
        ? cs.primary
        : Colors.orange.shade800;
    final hint = session.isLiveAt(now)
        ? 'Class is currently active'
        : session.isUpcomingAt(now)
        ? _startsInLabel(session.startTime.difference(now).inMinutes)
        : 'Class ended. Replay may be available.';
    return Container(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              _MiniPill(
                text: liveSessionMinutesLabel(session.durationMinutes),
                tone: cs.tertiary,
              ),
              _MiniPill(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 6),
              Text(
                liveSessionDateTime(session.startTime),
                style: TextStyle(
                  color: cs.primary,
                  fontWeight: FontWeight.w900,
                ),
              ...
              _DetailBlock(
                ...
                items: session.materials.isEmpty
                    ? ['No class material has been attached yet.']
                    : session.materials
                          .map(
                            (item) =>
                                '${item.title} — ${item.status.isEmpty ? item.subtitle : item.status}',
                          )
                          .toList(),
              ),
              const SizedBox(height: 12),
              _DetailBlock(
                ...
                items: [
                  session.chatEnabled
                      ? 'Class chat is enabled for learning interaction.'
                      : 'Class chat is locked for this session.',
                  session.questionsEnabled
                      ? 'Q&A is enabled for student questions.'
                      : 'Q&A is locked for this session.',
                  session.attendanceEnabled
                      ? 'Attendance is active for this live class.'
                      : 'Attendance is not active for this class.',
                ],
              ),
            ],
          ),
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Icon(icon, size: 17, color: cs.onSurface.withValues(alpha: 0.55)),
...
class _MiniPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text(
      text,
      style: TextStyle(color: tone, fontWeight: FontWeight.w900, fontSize: 11),
    ),
  );
}
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Live classes published for your registered courses will appear here.',
