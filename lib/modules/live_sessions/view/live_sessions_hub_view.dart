...
class _LiveSessionsHubViewState extends State<LiveSessionsHubView> {
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      if (visibleSessions.isEmpty)
                        ...
                        ...visibleSessions.map(
                          (session) => _LiveClassCard(
                            session: session,
                            onJoin: () => _joinSession(session, profile),
                            onDetails: () => _showDetails(session),
                          ),
                        ),
                    ],
                  ),
  ...
  void _showDetails(LiveSessionModel session) {
    Get.bottomSheet<void>(
      _LiveClassDetailsSheet(session: session),
...
class _LiveClassHero extends StatelessWidget {
  const _LiveClassHero({required this.studentName, required this.onBack});
...
class _MetricCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: cs.surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: cs.onSurface.withValues(alpha: 0.07))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Icon(icon, color: cs.primary, size: 20),
        const SizedBox(height: 8),
        Text(value, style: TextStyle(color: cs.primary, fontWeight: FontWeight.w900, fontSize: 20)),
        const SizedBox(height: 2),
        Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: cs.onSurface.withValues(alpha: 0.62), fontWeight: FontWeight.w800, fontSize: 12)),
      ]),
    );
  }
}
...
class _ReadinessCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          CircleAvatar(backgroundColor: cs.primary.withValues(alpha: 0.10), child: Icon(Icons.fact_check_outlined, color: cs.primary)),
          const SizedBox(width: 10),
          Expanded(child: Text('Class readiness', style: TextStyle(color: cs.onSurface, fontWeight: FontWeight.w900, fontSize: 16))),
          _MiniPill(text: 'Student checklist', tone: cs.primary),
        ]),
        const SizedBox(height: 12),
        const _CheckLine(text: 'Use a stable network before joining live class.'),
        const _CheckLine(text: 'Keep microphone muted until the lecturer allows questions.'),
        const _CheckLine(text: 'Join with your registered student profile for attendance.'),
      ]),
    );
  }
}
...
class _FilterBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: filters.map((filter) => Padding(padding: const EdgeInsets.only(right: 8), child: ChoiceChip(label: Text(filter), selected: selected == filter, onSelected: (_) => onChanged(filter)))).toList(),
      ),
    );
  }
}
...
class _LiveClassCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: cs.surface, borderRadius: BorderRadius.circular(20), border: Border.all(color: tone.withValues(alpha: 0.14)), boxShadow: [BoxShadow(blurRadius: 16, offset: const Offset(0, 8), color: cs.shadow.withValues(alpha: 0.04))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(width: 50, height: 50, decoration: BoxDecoration(color: tone.withValues(alpha: 0.10), borderRadius: BorderRadius.circular(16)), child: Icon(session.isLiveAt(now) ? Icons.video_camera_front_rounded : Icons.live_tv_outlined, color: tone)),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(session.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: cs.onSurface.withValues(alpha: 0.70), fontWeight: FontWeight.w600, height: 1.30)),
          ])),
          _MiniPill(text: status, tone: tone),
        ]),
        const SizedBox(height: 12),
        Wrap(spacing: 8, runSpacing: 8, children: [
          _MiniPill(text: liveSessionDayTimeRange(session.startTime, session.endTime), tone: cs.primary),
          _MiniPill(text: session.roomLabel, tone: cs.secondary),
          _MiniPill(text: liveSessionMinutesLabel(session.durationMinutes), tone: cs.tertiary),
          _MiniPill(text: session.attendanceEnabled ? 'Attendance active' : 'Attendance off', tone: cs.primary),
        ]),
        const SizedBox(height: 10),
        _InfoRow(icon: Icons.person_outline_rounded, text: 'Lecturer: ${session.lecturerName}'),
        _InfoRow(icon: Icons.timer_outlined, text: hint),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: FilledButton.icon(onPressed: onJoin, icon: Icon(session.isCompletedAt(now) ? Icons.replay_rounded : Icons.video_call_outlined), label: Text(session.isCompletedAt(now) ? 'Open replay' : session.isLiveAt(now) ? 'Join now' : 'Open lobby'))),
          const SizedBox(width: 10),
          Expanded(child: OutlinedButton.icon(onPressed: onDetails, icon: const Icon(Icons.info_outline_rounded), label: const Text('Details'))),
        ]),
      ]),
    );
  }
      ...
      child: SafeArea(
        ...
        child: SingleChildScrollView(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('${session.courseCode} • ${session.title}', style: TextStyle(color: cs.onSurface, fontWeight: FontWeight.w900, fontSize: 20)),
            const SizedBox(height: 6),
            Text(liveSessionDateTime(session.startTime), style: TextStyle(color: cs.primary, fontWeight: FontWeight.w900)),
            const SizedBox(height: 12),
            _DetailBlock(title: 'Class agenda', icon: Icons.format_list_bulleted_rounded, items: session.agenda),
            const SizedBox(height: 12),
            _DetailBlock(title: 'Class materials', icon: Icons.folder_copy_outlined, items: session.materials.isEmpty ? ['No class material has been attached yet.'] : session.materials.map((item) => '${item.title} — ${item.status.isEmpty ? item.subtitle : item.status}').toList()),
            const SizedBox(height: 12),
            _DetailBlock(title: 'Student rules', icon: Icons.rule_folder_outlined, items: [
              session.chatEnabled ? 'Class chat is enabled for learning interaction.' : 'Class chat is locked for this session.',
              session.questionsEnabled ? 'Q&A is enabled for student questions.' : 'Q&A is locked for this session.',
              session.attendanceEnabled ? 'Attendance is active for this live class.' : 'Attendance is not active for this class.',
            ]),
          ]),
        ),
      ),
...
class _DetailBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: cs.onSurface.withValues(alpha: 0.035), borderRadius: BorderRadius.circular(20), border: Border.all(color: cs.onSurface.withValues(alpha: 0.06))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        ...items.map((item) => Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(Icons.check_circle_outline_rounded, color: cs.primary, size: 18), const SizedBox(width: 8), Expanded(child: Text(item, style: TextStyle(color: cs.onSurface.withValues(alpha: 0.74), fontWeight: FontWeight.w700, height: 1.30)))]))),
      ]),
    );
  }
}
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [Icon(icon, size: 17, color: cs.onSurface.withValues(alpha: 0.55)), const SizedBox(width: 7), Expanded(child: Text(text, style: TextStyle(color: cs.onSurface.withValues(alpha: 0.68), fontWeight: FontWeight.w700)))]));
  }
}
...
class _MiniPill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6), decoration: BoxDecoration(color: tone.withValues(alpha: 0.10), borderRadius: BorderRadius.circular(999), border: Border.all(color: tone.withValues(alpha: 0.14))), child: Text(text, style: TextStyle(color: tone, fontWeight: FontWeight.w900, fontSize: 11)));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        Icon(Icons.live_tv_outlined, size: 44, color: cs.primary),
        const SizedBox(height: 10),
        Text('No $filter live class', style: TextStyle(color: cs.onSurface, fontWeight: FontWeight.w900, fontSize: 16)),
        const SizedBox(height: 7),
        Text('Live classes published for your registered courses will appear here.', textAlign: TextAlign.center, style: TextStyle(color: cs.onSurface.withValues(alpha: 0.68), fontWeight: FontWeight.w600, height: 1.30)),
      ]),
    );
  }
}
