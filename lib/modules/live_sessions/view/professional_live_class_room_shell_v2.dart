...
enum _ClassPanelMode { floating, minimized, fullscreen }

class ProfessionalLiveClassRoomShellV2 extends StatefulWidget {
  const ProfessionalLiveClassRoomShellV2({super.key});

  @override
  State<ProfessionalLiveClassRoomShellV2> createState() =>
      _ProfessionalLiveClassRoomShellV2State();
}

class _ProfessionalLiveClassRoomShellV2State
    extends State<ProfessionalLiveClassRoomShellV2> {
  ...
  _ClassPanelMode _panelMode = _ClassPanelMode.floating;
  Offset _panelOffset = const Offset(24, 92);
  ...
  Future<void> _openRoom() async {
    ...
    if (room != null) _syncBoardPages(room);
  }

  void _syncBoardPages(LiveSessionRoomState room) {
  ...
  Future<void> _setStageFullscreen(bool value) async {
    if (_layoutController.isFullscreen != value) {
      _layoutController.toggleFullscreen();
    }
    await _applyFullscreen(value || _panelMode == _ClassPanelMode.fullscreen);
  }

  Future<void> _setPanelMode(_ClassPanelMode mode) async {
    setState(() => _panelMode = mode);
    await _applyFullscreen(
      mode == _ClassPanelMode.fullscreen || _layoutController.isFullscreen,
    );
  }

  Future<void> _applyFullscreen(bool value) {
    return SystemChrome.setEnabledSystemUIMode(
      value ? SystemUiMode.immersiveSticky : SystemUiMode.edgeToEdge,
    );
  }

  Future<void> _toggleMic(bool value) => _controller.toggleMicrophone(value);

  Future<void> _toggleCamera(bool value) async {
    await _controller.toggleCamera(value);
    final room = _controller.room.value;
    if (room != null) _syncBoardPages(room);
  }
            ...
            builder: (context, _) {
              return LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final liveFullscreen = _panelMode == _ClassPanelMode.fullscreen;
                              ...
                              if (!hideChrome)
                                Padding(
                                  ...
                                  child: _Header(
                                    ...
                                    isConnecting: _controller.isConnectingMedia.value,
                                    onFullscreen: () => _setStageFullscreen(true),
                                    onLeave: _leave,
                                  ),
                                ),
                              Expanded(
                                child: Padding(
                                  ...
                                  child: _Stage(
                                    ...
                                    onStageFullscreen: () => _setStageFullscreen(
                                      !_layoutController.isFullscreen,
                                    ),
                                  ),
                                ),
                              ),
                              if (!hideChrome)
                                Padding(
                                  ...
                                  child: _BottomBar(
                                    ...
                                    onMic: () => _toggleMic(!micOn),
                                    onCamera: () => _toggleCamera(!cameraOn),
                                    onChat: () => _showChat(room),
                                    onPeople: () => _showPeople(room),
                                    onQuestion: () => _showQuestion(),
                                    onRecord: () => _controller.toggleRecording(!recordingOn),
                                    onClassFullscreen: () => _setPanelMode(
                                      _ClassPanelMode.fullscreen,
                                    ),
                                    onLeave: _leave,
                                  ),
                                ),
  ...
  Widget _buildFloatingPanel({
    ...
    if (_panelMode == _ClassPanelMode.fullscreen) {
      return Positioned.fill(
        child: Padding(
          ...
          child: _LivePanel(
            ...
            participantResolver: _controller.mediaParticipantFor,
            isMobile: isMobile,
            onMinimize: () => _setPanelMode(_ClassPanelMode.minimized),
            onFullscreen: () => _setPanelMode(_ClassPanelMode.floating),
          ),
        ),
      );
    }
    ...
    final width = isMobile
        ? (_panelMode == _ClassPanelMode.minimized ? 172.0 : constraints.maxWidth * 0.82)
        : _panelSize.width.clamp(320.0, constraints.maxWidth * 0.58).toDouble();
    final height = isMobile
        ? (_panelMode == _ClassPanelMode.minimized ? 70.0 : 225.0)
    ...
    return Positioned(
      ...
      child: _LivePanel(
        ...
        onMinimize: () => _setPanelMode(
          _panelMode == _ClassPanelMode.minimized
              ? _ClassPanelMode.floating
              : _ClassPanelMode.minimized,
        ),
        onFullscreen: () => _setPanelMode(_ClassPanelMode.fullscreen),
      ),
    );
  }
  ...
  Future<void> _showChat(LiveSessionRoomState room) async {
  ...
  Future<void> _showPeople(LiveSessionRoomState room) async {
    await showModalBottomSheet<void>(
      context: context,
      showDragHandle: true,
      builder: (context) {
        final people = _participants(room);
        return SafeArea(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                trailing: Wrap(
                  spacing: 6,
                  children: [
                    Icon(item.micEnabled ? Icons.mic_rounded : Icons.mic_off_rounded),
                    Icon(
                      item.cameraEnabled
    ...
    return DecoratedBox(
      ...
      child: Padding(
        ...
        child: narrow
            ...
            : Row(
                children: [
                  Expanded(child: _TitleBlock(room: room)),
...
class _TitleBlock extends StatelessWidget {
  const _TitleBlock({required this.room});
          ...
          children: [
            if (!fullscreen) ...[
              _StageToolbar(
                ...
                onFullscreen: onStageFullscreen,
              ),
              const SizedBox(height: 12),
            ],
            Expanded(
                  ...
                  children: [
                    Positioned.fill(
                      child: layoutController.currentMode == LiveClassMode.video
                          ...
                          : _SlideSurface(material: material, room: room),
                    ),
...
class _SlideSurface extends StatelessWidget {
  const _SlideSurface({required this.material, required this.room});
...
class _LivePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    final minimized = mode == _ClassPanelMode.minimized;
    final fullscreen = mode == _ClassPanelMode.fullscreen;
...
class _FocusVideo extends StatelessWidget {
  const _FocusVideo({
    required this.lecturer,
    required this.participants,
    required this.participantResolver,
      ...
      child: Padding(
        ...
        child: SingleChildScrollView(
          ...
          child: Row(
            ...
            ].expand((item) => [item, const SizedBox(width: 8)]).toList(),
          ),
        ),
      ),
    );
...
class _More extends StatelessWidget {
  const _More({required this.actions});
