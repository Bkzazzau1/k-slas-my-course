...
void main() {
  ...
  test('adapter should attach evidence manifest to high-risk camera events', () async {
    ...
    final entries = IntegrityLedgerService.pendingLedgerEntries('KASU/CSC/001');
  ...
  test('adapter should attach audio evidence request for voice events', () async {
    final engine = LocalAiEngine();
    final controller = ProctoringController();
    final evidenceService = EvidenceCaptureService();
    final adapter = LocalAiProctoringAdapter(
    ...
    final manifests = evidenceService.loadManifests('KASU/CSC/001');
    expect(manifests, hasLength(1));
    expect(manifests.first.artifacts.any((artifact) => artifact.kind == 'audioClip'), isTrue);
